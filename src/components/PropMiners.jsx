...
export default function PropMiners({visible}) {
    const navigate = useNavigate();
    const [data, setData] = useState({})
    const fetchData = async () => {
    try {
      const response = await axios.get("https://api-bugna.xpoolminer.com/api/v2/prop/miners");
  ...
  return (
    ...
    <Header></Header>
      ...
      <div className="flex flex-row w-4/5 h-auto ml-32 mt-2" >
        ...
        <NavLink 
          ...
          "inline-flex w-96 h-10 m-auto text-center font-medium justify-center gap-x-1 border-b-2 border-transparent text-white text-base"
          }
        >
    ...
    <table className='table-auto text-gray-300 w-4/5 h-auto m-auto text-center justify-center' >
            ...
            <tbody id="tabs-with-underline-2" role="tabpanel" aria-labelledby="tabs-with-underline-item-2">
              {data.miners && Object.keys(data.miners).map((k) => {
                ...
                return (
                  <tr key={k} className="bg-gray-700">
                    ...
                    {/* <td className="text-white font-mono">10000000</td> */}
